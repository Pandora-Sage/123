...
if __name__ == '__main__':
    ...
    def objective(trial):
        hyp_params.attn_dropout = trial.suggest_float('attn_dropout', 0.2, 0.5)
        hyp_params.embed_dropout = trial.suggest_float('embed_dropout', 0.3, 0.5)
        hyp_params.out_dropout = trial.suggest_float('out_dropout', 0.15, 0.45)
        hyp_params.l_kernel_size = 1  # 锁定
        hyp_params.a_kernel_size = trial.suggest_int('a_kernel_size', 2, 4, step=2)
        hyp_params.multimodal_layers = trial.suggest_int('multimodal_layers', 2, 4, step=1)
        if dataset not in ['meld_senti', 'meld_emo']:
            hyp_params.v_kernel_size = trial.suggest_int('v_kernel_size', 1, 3, step=2)

        # MPLMM 证明高 miss_prob 更好，放宽到 0.3-0.7
        hyp_params.miss_prob = trial.suggest_float('miss_prob', 0.3, 0.7)
        hyp_params.lambda_gen = trial.suggest_float('lambda_gen', 0.3, 0.7)
