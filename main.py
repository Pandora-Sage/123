...
if __name__ == '__main__':
    ...
    def objective(trial):
        ...
        hyp_params.embed_dropout = trial.suggest_float('embed_dropout', 0.1, 0.5)
        hyp_params.out_dropout = trial.suggest_float('out_dropout', 0.1, 0.4)
        hyp_params.l_kernel_size = trial.suggest_int('l_kernel_size', 1, 3, step=2)
        if dataset != 'meld_senti' and dataset != 'meld_emo':
            hyp_params.v_kernel_size = trial.suggest_int('v_kernel_size', 1, 5, step=2)
        hyp_params.a_kernel_size = trial.suggest_int('a_kernel_size', 2, 6, step=2)
        hyp_params.multimodal_layers = trial.suggest_int('multimodal_layers', 2, 8, step=1)
