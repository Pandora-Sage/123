...
class CFMModel(nn.Module):
    ...
    def forward(self, x_l, x_a, x_v=None, phase='train', missing_flags=None,
        ...
        if self.use_gate:
            ...
            if flags['L'].any():
                conf_l = self.uncertainty_head(h_l.mean(dim=0))
            ...
            if flags['A'].any():
                conf_a = self.uncertainty_head(h_a.mean(dim=0))
            ...
            if h_v is not None and flags['V'].any():
                conf_v = self.uncertainty_head(h_v.mean(dim=0))
        ...
        if self.is_meld:
            # 缺失的模态用零填充，防止信息泄露
            x_l_for_cls = x_l_encoded.clone()
            x_a_for_cls = x_a.clone()
            if flags['L'].any():
                mask_l = flags['L'].bool().unsqueeze(-1).unsqueeze(-1)  # (batch, 1, 1)
                x_l_for_cls = x_l_for_cls * (~mask_l).float()
            if flags['A'].any():
                mask_a = flags['A'].bool().unsqueeze(-1).unsqueeze(-1)
                x_a_for_cls = x_a_for_cls * (~mask_a).float()
            x_l_t = F.dropout(x_l_for_cls.transpose(1, 2), p=self.embed_dropout, training=self.training)
            x_a_t = x_a_for_cls.transpose(1, 2)
            cls_token = self.proj_cls(torch.cat((x_l_t, x_a_t), dim=1)).permute(2, 0, 1) * scale
        else:
