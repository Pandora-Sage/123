...
def unpack_batch(batch, hyp_params):
    ...
    if is_meld:
        # MELD TensorDataset: [audio, text, mask, labels]
        audio = batch[0]     # (batch, 33, audio_dim)
        text = batch[1]      # (batch, 33, text_dim)
...
def _save_trial_log(hyp_params, all_results):
    ...
    with open(summary_file, 'a') as f:
        if write_header:
            if is_meld:
                headers = [
                    ...
                    'ml', 'ak', 'ed', 'pl', 'attn_drop', 'embed_drop', 'out_drop'
                ]
            else:
                headers = [
                    ...
                    '-L(AV)', '-LV(A)', '-LA(V)', 'Avg7',
                    'miss_prob', 'lambda_gen', 'lambda_aux',
                    'ml', 'ak', 'vk', 'ed', 'pl', 'attn_drop', 'embed_drop', 'out_drop'
                ]
        ...
        vals += [
            str(getattr(hyp_params, 'embed_dim', 32)),
            str(getattr(hyp_params, 'prompt_len', 4)),
